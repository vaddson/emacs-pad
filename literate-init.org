#+title: Конфигурация Emacs под Termux (для планшета)
#+startup: content
#+startup: noindent

* Вводная

Конфигурация Emacs для планшета (Android) в терминале Termux.

Нужна в качестве мобильного заметочника на базе Org-Roam.
Также включает в себя орфографию, словарик и калькулятор.
...
4. Установить Emacs:
   : pkg install emacs
5. Установить openssl:
   : pkg install openssl
6. Установить RipGrep:
   : pkg install ripgrep
7. Установить словарик Hunspell:
   : pkg install hunspell
8. Установить словарик StarDict:
   : pkg install sdcv
9. Установить Python для живого калькулятора:
   : pkg install python
   : pkg install x11-repo
   : pkg install tigervnc
10. Если отсутствуют, то установить также
    gcc, g++ и git.
11. Для удобства в файле настроек [[~/.termux/termux.properties]]
    прописать виртуальные клавиши:
    #+begin_src conf
extra-keys = [['ESC',{key:'CTRL',display:C},{key:'ALT',display:M},':','~','{'   ,'UP'  ,'}'    ,'['], \
              ['-'  ,     'TAB'            ,     '/'             ,';','`','LEFT','DOWN','RIGHT',']']]
    #+end_src

*Установка конфигурации Emacs*
...
На планшете удобно работать в Emacs с раскладкой Vim (пакет evil). Но
если это не по душе, то можно вернуть родную раскладку следующим
образом:
...
(defun my/workspace-dictionary ()
    ...
    (let* ((name "Dictionary")
        ...
        (if (member name persp-names-cache)
            ...
            (progn (my/set-title-bar-and-workspace-interactive name)
...
Чтобы все работало в Termux, нужно сделать две установки:
: pkg install x11-repo
: pkg install tigervnc
После этого в Python станет доступен импорт модуля turtle,
необходимого для пакета live-py-mode.

Установим пакет =live-py-mode=.
...
Если перед созданием калькулятора в активном буфере был выделен текст,
то этот текст будет добавлен в буфер 'калькулятора'. Результат кода
ниже - функция, которая открывает live-python буфер с калькулятором.

#+begin_src emacs-lisp :tangle yes
(defun my/calculator-python-open ()
    "Open 'calulator' on live-py-mode."
    (interactive)
...
Осталось сделать функцию, которая создает рабочее пространство с
калькулятором.

#+begin_src emacs-lisp :tangle yes
(defun my/calculator-python-persp-open ()
    (interactive)
    (let* ((name "Calculator"))
...
#+begin_src emacs-lisp :tangle yes
(my/leader-def nil 'override
    "o c" '(my/calculator-python-persp-open :which-key "Calculator live on Python"))
#+end_src
